...
public class Main {
    ...
    private void doControlBreak(){
        ArrayList<Meal> meals=cookbook.getMeals();
        MealType oldMealType=null;
        int count=0;
        int totalCalories=0;
        System.out.printf("%-15s%-15s%-15s\n","Meal Type","Total","Mean");
        for (Meal meal: meals) {
            if(oldMealType !=null && meal.getMealType()!=oldMealType){
                System.out.printf("%-15s%-15d%-15d\n",oldMealType,totalCalories,(totalCalories/count));
                count=0;
                totalCalories=0;
            }
            count++;
            totalCalories += meal.getCalories();
            oldMealType = meal.getMealType();
        }
    }
}
